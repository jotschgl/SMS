...
public class CompetitionManager {
    ...
    public void updateExistingCompetition(Competition competition) {
        ...
        newCompetitionTeams.addAll(competition.getCompetitionTeams());
        for (CompetitionTeam team : competition.getCompetitionTeams()) {
            if (team.getId() != -1) {
                if (team.getShouldBeDeleted()) {
                    persistenceManager.delete(team);
                    newCompetitionTeams.remove(team);
                }
            }
        }
        competition.setCompetitionTeams(newCompetitionTeams);
